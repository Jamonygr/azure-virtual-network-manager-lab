...
```mermaid
flowchart LR
    appsnet["App snet-workload"] --- rt
    datasnet["Data snet-workload"] --- rt
    rt["AVNM managed route table<br/>ManagedOnly, BGP propagation off"]
    rt -->|"203.0.113.0/24"| hole(("NoNextHop<br/>blackhole"))
    hubsnet["Hub snet-management"] -.-|"not targeted"| rt

    style hubsnet stroke-dasharray: 5 5
```
