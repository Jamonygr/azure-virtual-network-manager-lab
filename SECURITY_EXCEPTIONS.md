...
- No network security group is attached because no NIC or workload exists. Azure Virtual Network Manager security admin rules are the subject under test. `.checkov.yml` carries the narrowly scoped Checkov `CKV2_AZURE_31` exception; Trivy exceptions `AVD-AZU-0047` and `AVD-AZU-0048` are recorded in `.trivyignore.yaml`.
